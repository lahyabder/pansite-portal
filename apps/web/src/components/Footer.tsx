                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10 lg:gap-8">
                            ...
                            {Object.entries(socialLinks).map(([platform, url]) => (
                                url && (
                                    <a
                                        ...
                                        className="w-10 h-10 bg-white/5 hover:bg-pan-gold/20 border border-white/10 rounded-xl flex items-center justify-center text-white/80 hover:text-pan-gold transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-pan-gold focus-visible:ring-offset-2 focus-visible:ring-offset-pan-navy"
                                        aria-label={platform}
                                    >
                    ...
                    <div>
                        ...
                        <ul className="space-y-3">
                            {column1.map((item) => (
                                <li key={item.href}>
                                    <Link href={item.href} className="text-white/80 text-sm hover:text-white transition-colors flex items-center gap-2 group focus:outline-none focus-visible:ring-2 focus-visible:ring-pan-gold focus-visible:ring-offset-2 focus-visible:ring-offset-pan-navy rounded-md px-1 -mx-1">
                    ...
                    <div>
                        ...
                        <ul className="space-y-4 text-white/90 text-sm">
                            ...
                            <li className="flex items-center gap-3">
                                ...
                                <a href={`tel:${(settings?.contactPhones?.[0] || dict.footer.phone).replace(/\s/g, '')}`} className="hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-pan-gold focus-visible:ring-offset-2 focus-visible:ring-offset-pan-navy rounded-md px-1 -mx-1"><span dir="ltr">{settings?.contactPhones?.[0] || dict.footer.phone}</span></a>
                            </li>
                            <li className="flex items-center gap-3">
                                <svg className="w-4 h-4 shrink-0 text-pan-gold/40" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                                </svg>
                                <a href={`mailto:${settings?.contactEmails?.[0] || dict.footer.email}`} className="hover:text-white transition-colors uppercase text-xs font-bold focus:outline-none focus-visible:ring-2 focus-visible:ring-pan-gold focus-visible:ring-offset-2 focus-visible:ring-offset-pan-navy rounded-md px-1 -mx-1">{settings?.contactEmails?.[0] || dict.footer.email}</a>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div className="border-t border-white/5 bg-black/10">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 md:py-8">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex flex-col items-center md:items-start gap-1 text-[10px] text-white/70 uppercase font-black tracking-widest">
                            ...
                            <p className="flex items-center gap-1.5 opacity-50">
                                Powered by <a href="https://afrikyia.com" target="_blank" className="text-pan-gold hover:text-white transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-pan-gold rounded-sm px-1">Afrikyia</a>
                            </p>
                        </div>
                        <div className="flex flex-wrap items-center justify-center gap-6">
                            {legalLinks.map((item) => (
                                <Link key={item.href} href={item.href} prefetch={false} className="text-[10px] text-white/70 hover:text-white transition-colors font-bold uppercase tracking-widest focus:outline-none focus-visible:ring-2 focus-visible:ring-pan-gold focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-sm px-1">
                                    {item.label}
                                </Link>
                            ))}
                        </div>
