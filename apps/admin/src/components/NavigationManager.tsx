  ...
  return (
    <div className="space-y-8 pb-20">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="font-outfit text-3xl font-black text-slate-900">Gestion de la Navigation</h1>
          <p className="text-slate-500 mt-2 font-medium">Structurez les menus et liens de votre portail.</p>
        </div>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl font-bold text-sm transition-all ${
                  activeLocation === loc.id 
                    ...
                    : 'bg-white border border-gray-200 text-slate-500 hover:text-slate-900'
                }`}
              >
        ...
        <div className="lg:col-span-3 space-y-6">
           <div className="glass-card rounded-[2.5rem] p-10">
              <div className="flex items-center justify-between mb-8">
                 <h2 className="text-xl font-bold text-slate-900">Items du {MENU_LOCATIONS.find(l=>l.id===activeLocation)?.label}</h2>
                 <button 
                  onClick={addItem}
                  className="p-2.5 bg-gray-100 border border-gray-200 rounded-xl text-sky-600 hover:bg-sky-500 hover:text-slate-900 transition-all"
                 >
              ...
              <div className="space-y-3">
                 ...
                 {menu.items.map((item: any, idx: number) => (
                   <div key={item.id || idx} className="flex items-center gap-4 bg-white/50 border border-gray-200 p-4 rounded-2xl group group-hover:border-gray-300 transition-all">
                      <button className="p-2 text-slate-700 hover:text-slate-500 cursor-grab">
                      ...
                      <div className="grid grid-cols-2 gap-4 flex-1">
                         <div className="space-y-1.5">
                            ...
                            <input 
                              ...
                              className="w-full bg-white border border-gray-200 rounded-xl px-3 py-1.5 text-xs text-slate-900 outline-none"
                            />
                         </div>
                         <div className="space-y-1.5">
                            ...
                            <input 
                              ...
                              className="w-full bg-white border border-gray-200 rounded-xl px-3 py-1.5 text-xs text-sky-600 outline-none"
                            />
                         </div>
                      </div>
                      <div className="flex items-center gap-2">
                         <button className="p-2 text-slate-700 hover:text-slate-900 transition-colors"><Globe className="w-4 h-4" /></button>
                         <button 
                           onClick={() => {
                             const next = {...menu};
           ...
           <div className="bg-sky-500/5 border border-sky-500/10 rounded-3xl p-8 flex items-start gap-4">
              ...
              <div>
                 <h4 className="font-bold text-slate-900">Note sur les sous-menus</h4>
                 <p className="text-sm text-slate-500 mt-1">Le système supporte actuellement la navigation à un seul niveau pour le menu principal. Les sous-menus seront activés lors d'une prochaine mise à jour.</p>
              </div>
           </div>
        </div>
      </div>
