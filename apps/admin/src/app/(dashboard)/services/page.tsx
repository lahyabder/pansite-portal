...
import { mockServices, t } from '@pan/shared';
import type { Service } from '@pan/shared';
        ...
        <RequirePermission module="services">
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-admin-text">Catalogue des services</h2>
                        <p className="text-admin-text-muted text-sm mt-1">{mockServices.length} service(s) actif(s)</p>
                    </div>
                    {can('services', 'create') && (
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-admin-border">
                                    ...
                                    <th className="text-start px-5 py-3.5 text-admin-text-muted text-xs font-semibold uppercase tracking-wider">Service</th>
                                    <th className="text-start px-5 py-3.5 text-admin-text-muted text-xs font-semibold uppercase tracking-wider">Direction</th>
                            ...
                            <tbody className="divide-y divide-admin-border">
                                {mockServices.map((svc) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="text-admin-text text-sm font-medium">{svc.name.fr}</div>
                                        ...
                                        <td className="px-5 py-4 text-end">
                                            <button className="text-admin-text-muted hover:text-admin-text text-sm" onClick={(e) => { e.stopPropagation(); setSelected(svc.id); }}>Voir</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        ...
                        <div className="col-span-3 bg-admin-surface rounded-xl border border-admin-border overflow-auto max-h-[75vh]">
                            <div className="p-6 space-y-5">
                                <div className="flex items-start justify-between">
                                    <div>
                                        <h3 className="text-lg font-bold text-admin-text">{service.name.fr}</h3>
                                ...
                                {/* Description */}
                                <div>
                                    <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Description</h4>
                                    <p className="text-admin-text text-sm">{service.description.fr}</p>
                                    {service.longDescription && <p className="text-admin-text-muted text-sm mt-2">{service.longDescription.fr}</p>}
                                </div>

                                {/* Features */}
                                <div>
                                    <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Points forts ({service.features.length})</h4>
                                    <div className="flex flex-wrap gap-2">
                                        {service.features.map((f, i) => (
                                            <span key={i} className="px-3 py-1 bg-admin-surface-alt rounded-lg text-admin-text text-xs">{f.fr}</span>
                                        ))}
                                    </div>
                                </div>
                                ...
                                {service.beneficiaries && (
                                    <div>
                                        <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Bénéficiaires</h4>
                                        <ul className="space-y-1">
                                            {service.beneficiaries.map((b, i) => (
                                                <li key={i} className="text-admin-text text-sm flex items-center gap-2">
                                                    <span className="w-1.5 h-1.5 bg-admin-primary rounded-full" /> {b.fr}
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                ...
                                {service.steps && (
                                    <div>
                                        <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Procédure ({service.steps.length} étapes)</h4>
                                        <div className="space-y-2">
                                            {service.steps.map((step, i) => (
                                                <div key={i} className="flex gap-3 bg-admin-surface-alt rounded-lg p-3">
                                                    ...
                                                    <div>
                                                        <div className="text-admin-text text-sm font-medium">{step.title.fr}</div>
                                                        <div className="text-admin-text-muted text-xs mt-0.5">{step.description.fr}</div>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                ...
                                {service.requiredDocuments && (
                                    <div>
                                        <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Documents requis</h4>
                                        <ul className="space-y-1">
                                            {service.requiredDocuments.map((d, i) => (
                                                <li key={i} className="text-admin-text text-sm flex items-center gap-2">📄 {d.fr}</li>
                                            ))}
                                        </ul>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {service.deadline && (
                                        <div className="bg-admin-surface-alt rounded-lg p-3">
                                            <div className="text-admin-text-muted text-xs uppercase tracking-wider mb-1">Délai</div>
                                            <div className="text-admin-text text-sm">{service.deadline.fr}</div>
                                        </div>
                                    )}
                                    {service.costs && (
                                        <div className="bg-admin-surface-alt rounded-lg p-3">
                                            <div className="text-admin-text-muted text-xs uppercase tracking-wider mb-1">Coûts</div>
                                            <div className="text-admin-text text-sm">{service.costs.fr}</div>
                                        </div>
                                    )}
                                </div>
                                ...
                                {service.contactPoints && (
                                    <div>
                                        <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Points de contact</h4>
                                        {service.contactPoints.map((cp, i) => (
                                            <div key={i} className="bg-admin-surface-alt rounded-lg p-3 text-sm space-y-1">
                                                <div className="text-admin-text font-medium">{cp.name.fr}</div>
                                                {cp.phone && <div className="text-admin-text-muted">📞 {cp.phone}</div>}
                                                {cp.email && <div className="text-admin-primary">✉️ {cp.email}</div>}
                                                {cp.hours && <div className="text-admin-text-muted">🕐 {cp.hours.fr}</div>}
                                            </div>
                                        ))}
                                    </div>
                                )}
