...
const statusConfig: Record<RequestStatus, { label: string; color: string; bg: string }> = {
    new: { label: 'Nouvelle', color: 'text-blue-400', bg: 'bg-blue-500/15' },
    assigned: { label: 'Assignée', color: 'text-purple-400', bg: 'bg-purple-500/15' },
    in_progress: { label: 'En cours', color: 'text-amber-400', bg: 'bg-amber-500/15' },
    waiting_more_info: { label: 'En attente info', color: 'text-orange-400', bg: 'bg-orange-500/15' },
    approved: { label: 'Approuvée', color: 'text-emerald-400', bg: 'bg-emerald-500/15' },
    rejected: { label: 'Rejetée', color: 'text-red-400', bg: 'bg-red-500/15' },
    closed: { label: 'Clôturée', color: 'text-gray-400', bg: 'bg-gray-500/15' },
};

const typeConfig: Record<RequestType, { label: string; icon: string }> = {
    information: { label: "Demande d'info", icon: 'ℹ️' },
    reclamation: { label: 'Réclamation', icon: '⚠️' },
    document_request: { label: 'Demande doc.', icon: '📄' },
    rendez_vous: { label: 'Rendez-vous', icon: '📅' },
};
        ...
        <RequirePermission module="requests">
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-admin-text">File des demandes</h2>
                        <p className="text-admin-text-muted text-sm mt-1">{stats.total} demande(s) · Temps moyen de traitement : {stats.avgProcessingHours}h</p>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-4 gap-3">
                        {stats.byService.map(s => (
                            <div key={s.serviceId} className="bg-admin-surface rounded-xl p-3 border border-admin-border">
                                ...
                                <div className="text-lg font-bold text-admin-text mt-0.5">{s.count} <span className="text-xs text-admin-text-muted font-normal">demande(s)</span></div>
                            </div>
                        ))}
                    </div>
                )}
                ...
                <div className="flex gap-3 items-center">
                    <input
                        type="text"
                        placeholder="Rechercher (sujet, nom, référence)..."
                        value={search}
                ...
                <div className="grid grid-cols-5 gap-4">
                    ...
                    <div className={`${selected ? 'col-span-2' : 'col-span-5'} bg-admin-surface rounded-xl border border-admin-border divide-y divide-admin-border overflow-auto max-h-[65vh]`}>
                        {filtered.length === 0 ? (
                            <div className="p-10 text-center text-admin-text-muted text-sm">Aucune demande trouvée.</div>
                        ) : (
                            filtered.map((req) => {
                        ...
                        <div className="col-span-3 bg-admin-surface rounded-xl border border-admin-border overflow-auto max-h-[65vh]">
                            <div className="p-6 space-y-5">
                                ...
                                <div className="bg-admin-surface-alt rounded-xl p-4 space-y-1.5">
                                    <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Demandeur</h4>
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        <div><span className="text-admin-text-muted">Nom:</span> <span className="text-admin-text">{selected.senderName}</span></div>
                                    ...
                                    {selected.serviceName && (
                                        <div className="text-sm mt-1"><span className="text-admin-text-muted">Service lié:</span> <span className="text-admin-primary">{selected.serviceName}</span></div>
                                    )}
                                </div>

                                {/* Message */}
                                <div>
                                    <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Message</h4>
                                ...
                                {selected.attachments.length > 0 && (
                                    <div>
                                        <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-2">Pièces jointes ({selected.attachments.length})</h4>
                                        <div className="space-y-2">
                                            {selected.attachments.map(att => (
                                                <div key={att.id} className="flex items-center gap-3 bg-admin-surface-alt rounded-lg p-3">
                                                    ...
                                                    <button className="text-admin-primary text-sm hover:underline">Télécharger</button>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                {selected.responseMessage && (
                                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4">
                                        <h4 className="text-xs font-semibold text-emerald-400 uppercase tracking-wider mb-2">💬 Réponse envoyée</h4>
                                ...
                                <div className="border-t border-admin-border pt-5 space-y-4">
                                    <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider">Actions</h4>
                                    ...
                                    <div className="flex gap-2">
                                        <textarea
                                            placeholder="Rédiger une réponse au demandeur..."
                                            value={responseText}
                                ...
                                <div className="border-t border-admin-border pt-5">
                                    <h4 className="text-xs font-semibold text-admin-text-muted uppercase tracking-wider mb-3">Historique</h4>
                                            ...
                                            return (
                                                <div key={entry.id} className="flex gap-3 text-sm">
                                                    ...
                                                    <div className="pb-3">
                                                        ...
                                                        <span className="text-admin-text-muted text-xs">{new Date(entry.createdAt).toLocaleString('fr')}</span>
                                                    </div>
                                                </div>
                                            );
                                        })}
